...
def interfase():
    ...
    if op1 == "0":
        ...
        for veiculo in result_cal_custo_viagem:
            for obj_veiculo in list_veiculos:
                if veiculo[0] == obj_veiculo.id:
                    headers = ["Modelo", "Marca", "Ano", "Cap.Tanque", "Auto.Cidade", "Auto.Estrada"]
                    table = [
                    [f"{obj_veiculo.modelo}", f"{obj_veiculo.marca}", f"{obj_veiculo.ano}",
                    f"{obj_veiculo.capa_tanque:.2f}L", f"{obj_veiculo.auto_cidade:.2f}Km", f"{obj_veiculo.auto_estrada:.2f}Km"]]
        ...
        while True:
            ...
            if status == "s":
                break
        result_tanque_cheio = preco_tanque_cheio([valor1, valor2], list_veiculos)
        result_percentual_diferencial = percentual_diferencial([valor1, valor2])
    return "Sucesso"
